...
function IssueForm({
  ...
  async function submit(formData: FormData) {
    ...
    setMessage(
      result.ok
        ? `Submitted: ${body.url ?? "issue created"}`
        : (body.error?.message ?? "Unable to submit."),
    );
    setPending(false);
  }
  return (
        ...
        <>
          ...
          <label>
            ...
            <select name="category">
              <option>Wrong phone model</option>
              <option>Incorrect camera/sensor placement</option>
              <option>Incorrect proportions</option>
              <option>Incorrect image quality</option>
              <option>Wrong alias</option>
              <option>Wrong model number</option>
              <option>Other</option>
            </select>
          </label>
        </>
